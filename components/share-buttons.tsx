...
export function ShareButtons({ className }: { className?: string }) {
  ...
  const recordShare = async (platform: string) => {
    try {
      console.log('[v0] Recording share:', platform)
      ...
      console.error('[v0] Error recording share:', error)
  ...
  return (
    <div className={cn('w-full', className)}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {channels.map((c) => {
          const Icon = c.icon
          const platformKey = c.label.toLowerCase()
          return (
            <a
              key={c.label}
              href={c.href}
              ...
              onClick={() => handleChannelShare(platformKey)}
              className={cn(
